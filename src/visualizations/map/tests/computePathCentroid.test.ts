...
describe('computeBoundingBoxCenter', () => {
  ...
  it('returns zero for empty path', () => {
    const center = computeBoundingBoxCenter('');
...
describe('computePolylabel', () => {
  ...
  it('Portugal polylabel is more central than polygon centroid', () => {
    ...
    const polylabel = computePolylabel(PORTUGAL_PATH);
    const bboxCenter = computeBoundingBoxCenter(PORTUGAL_PATH);
    ...
    const dist_poly_centroid = Math.sqrt((polylabel.x - centroid.x) ** 2 + (polylabel.y - centroid.y) ** 2);
    const dist_poly_bbox = Math.sqrt((polylabel.x - bboxCenter.x) ** 2 + (polylabel.y - bboxCenter.y) ** 2);
    ...
    expect(polylabel.y).toBeLessThan(-37);
    expect(polylabel.y).toBeGreaterThan(-42);
  });

  it('returns zero for empty path', () => {
    const pole = computePolylabel('');
    expect(pole.x).toBe(0);
    expect(pole.y).toBe(0);
  });

  it('returns zero for degenerate path', () => {
    const pole = computePolylabel('M 5 5');
    expect(pole.x).toBe(0);
    expect(pole.y).toBe(0);
  });
});
