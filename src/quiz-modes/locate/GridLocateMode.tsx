...
import { useGridLocateQuiz } from './useGridLocateQuiz';
import { GridLocateFeedback } from './GridLocateFeedback';
import styles from './LocateMode.module.css';

/**
 * Locate mode for grid visualizations (periodic table).
 * User clicks on a specific element to answer — only element clicks register.
 * Distance is measured as Manhattan distance in the "true" 32-column grid.
 */
export function GridLocateMode({
  ...
  }, [forceGiveUp, quiz.isFinished, quiz.handleGiveUp]); // eslint-disable-line react-hooks/exhaustive-deps
    ...
    <div className={styles.container}>
      ...
      <div className={styles.visualization}>
        <Renderer
          ...
          svgUnderlay={<GridLocateFeedback feedbackItems={quiz.feedbackItems} />}
        />
      </div>
    </div>
  );
...
function PromptDisplay({ targetLabel, currentIndex, total }: PromptDisplayProps) {
  return (
    <div className={styles.prompt}>
      ...
      >
        Click on <strong>{targetLabel}</strong>
      </motion.span>
    </div>
  );
}
